var view = view || {};
var grapher = grapher || window.grapher;
var python = python || window.python;
...
view.View = class {
    ...
    /**
     * entry
     * @param {onnx.Model} model 
     * @param {string} model name
     * @returns 
     */
    async open(model, modelName) {
        this.show('welcome spinner');
        this._sidebar.close();
        try {
            const graphs = Array.isArray(model.graphs) && model.graphs.length > 0 ? [model.graphs[0]] : [];
            await this._updateGraph(model, graphs);
            this._host.document.title = modelName;
        } catch (error) {
            throw error;
        }
    }
